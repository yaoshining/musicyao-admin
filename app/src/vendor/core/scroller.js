...
*/
(function($ , undefined) {
	var Ebp_Scroll = function(element , _settings) {
		var self = this;
		var settings = $.extend({}, $.fn.ebp_scroll.defaults, _settings);
		...
		this.create = function(_settings) {
			if(created) return;
			//if(disabled) return;
			if(_settings) settings = $.extend({}, $.fn.ebp_scroll.defaults, _settings);
			...
			this.$element.addClass('ebp-scroll '+((vertical? '' : ' scroll-hz') + (settings.styleClass ? ' '+settings.styleClass : '')));
			...
			if(settings.mouseWheel) {
				...
				this.$element.on('mousewheel.ebp_scroll DOMMouseScroll.ebp_scroll', function(event) {
					if(disabled) return;
					...
					if(mouse_track) {
						mouse_track = false;
						$('html').off('.ebp_scroll')
						$(mouse_release_target).off('.ebp_scroll');
			...
			// 	this.$element.on(event_name + '.ebp_scroll', function(event) {
			// 		if(disabled) {
			...
			if(settings.hoverReset) {
				...
				this.$element.on('mouseenter.ebp_scroll touchstart.ebp_scroll', function() {
					self.reset();
				})
			}
		...
		this.destroy = function() {
			...
			this.$element.removeClass('ebp-scroll scroll-hz'+(settings.extraClass ? ' '+settings.extraClass : ''));
			this.$element.off('.ebp_scroll')
		...
		function mouse_down_bar(e) {
			...
			mouse_track = true;
			$('html').off('mousemove.ebp_scroll').on('mousemove.ebp_scroll', mouse_move_bar)
			$(mouse_release_target).off('mouseup.ebp_scroll').on('mouseup.ebp_scroll', mouse_up_bar);
	...
	$.fn.ebp_scroll = function (option,value) {
		var retval;

		var $set = this.each(function () {
			var $this = $(this);
			var data = $this.data('ebp_scroll');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('ebp_scroll', (data = new Ebp_Scroll(this, options)));
	...
	/**
	$(document).on('ace.settings.ebp_scroll', function(e, name) {
		if(name == 'sidebar_collapsed') $('.ebp-scroll').scroller('reset');
	});
	$(window).on('resize.ebp_scroll', function() {
		$('.ebp-scroll').scroller('reset');
	});
	*/
